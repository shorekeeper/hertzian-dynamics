...
//!   * The CSV files dropped under hertzian_out/gpu_fft contain the
//!     CPU spectrum, the GPU spectrum, and the per-bin error so a
//!     plot can show where the deviation sits.
//!   * Round trip error should be similar in magnitude; a sharply
//!     larger value means the inverse path is misconfigured.
//!
//! No assertions on plot quality, only on numerical tolerance. If
//! the assert fires, the dumped CSVs are the first thing to open.
...
fn main() {
    ...
    // Build an input mixing two tones with f32 phase.
